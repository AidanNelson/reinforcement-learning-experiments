...
// import {CartPole, FlappyBird} from './cart_pole';
...
async function updateUIControlState() {
 ...
 async function setUpUI() {
  ...
  const cartPole = new FlappyBird(800,600);

  if (await SaveablePolicyNetwork.checkStoredModelStatus() != null) {
    policyNet = await SaveablePolicyNetwork.loadModel();
    logStatus('Loaded policy network from IndexedDB (see console for shape info)');
    policyNet.model.summary();
  ...
  testButton.addEventListener('click', async () => {
    ...
    while (!isDone) {
      ...
      tf.tidy(() => {
        ...
        let addtlReward;
        [isDone, addtlReward] = cartPole.update(action);
        // renderCartPole(cartPole, cartPoleCanvas);
        cartPole.show();
      });
      await tf.nextFrame();  // Unblock UI thread.
